...
class _RawOtpViewState extends State<RawOtpView>
    with RestorationMixin, WidgetsBindingObserver, _UtilsMixin
  ...
  late final _SelectionBuilder _gestureDetectorBuilder;
  ...
  TextEditingController get _effectiveController =>
      widget.inputController ?? _controller!.value;
  ...
  int get selectedIndex => text.length;

  String get text => _effectiveController.text;

  bool get _completed => text.length == widget.inputSize;

  @override
  void initState() {
    super.initState();
    _gestureDetectorBuilder = _SelectionBuilder(state: this);
    if (widget.inputController == null) {
      _createLocalController();
      _recentControllerValue = TextEditingValue(text: widget.text ?? "");
    } else {
      _recentControllerValue = _effectiveController.value.copyWith(
        text: widget.text,
      );
      widget.inputController!.addListener(_handleTextEditingControllerChanges);
    }
  ...
  void _maybeInitSmartAuth() async {
    ...
    final isAutofillEnabled =
        widget.smsAutofillMethod != SmsAutofillMethod.none;
  ...
  void _maybePrintAppSignature() async {
    if (widget.smsAutofillMethod == SmsAutofillMethod.smsRetrieverApi) {
      final res = await _smartAuth!.getAppSignature();
      debugPrint('OtpView: App Signature for SMS Retriever API Is: $res');
    }
  }

  void _listenForSmsCode() async {
    final useUserConsentApi =
        widget.smsAutofillMethod == SmsAutofillMethod.smsUserConsentApi;
    final res = await _smartAuth!.getSmsCode(
      useUserConsentApi: useUserConsentApi,
      matcher: widget.smsCodeMatcher,
      senderPhoneNumber: widget.senderPhoneNumber,
    );
    if (res.succeed && res.codeFound && res.code!.length == widget.inputSize) {
      _effectiveController.add(res.code!);
    }
    if (widget.listenForMultipleSms) {
      _listenForSmsCode();
    }
  }
  ...
  void _maybeValidateForm() {
    if (widget.autoValidateMode == AutoValidateMode.onSubmit) {
      _validator();
    }
  }
  ...
  @override
  void didUpdateWidget(RawOtpView oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.inputController == null && oldWidget.inputController != null) {
      _createLocalController(oldWidget.inputController!.value);
    } else if (widget.inputController != null &&
        oldWidget.inputController == null) {
    ...
    if (widget.inputController != oldWidget.inputController) {
      oldWidget.inputController
          ?.removeListener(_handleTextEditingControllerChanges);
      widget.inputController?.addListener(_handleTextEditingControllerChanges);
    }
  ...
  @override
  void dispose() {
    widget.inputController?.removeListener(_handleTextEditingControllerChanges);
    _focusNode?.dispose();
    _controller?.dispose();
  ...
  void _maybeCheckClipboard() async {
    if (widget.onClipboardFound != null) {
      ...
      if (clipboard.length == widget.inputSize) {
  ...
  String? _validator([String? _]) {
    final res = widget.inputValidator?.call(text);
            ...
            child: IgnorePointer(
              ...
              child: AnimatedBuilder(
                ...
                builder: (_, Widget? child) => Semantics(
                  maxValueLength: widget.inputSize,
    ...
    return RepaintBoundary(
      child: UnmanagedRestorationScope(
        ...
        child: EditableText(
          key: editableTextKey,
          maxLines: 1,
          style: _Constants._hiddenTextStyle,
          onChanged: (value) {
  ...
  Widget _buildFields() {
    Widget onlyFields() {
      return _SeparatedRaw(
        ...
        children: Iterable<int>.generate(widget.inputSize).map<Widget>((index) {
          return _Item(state: this, index: index);
        }).toList(),
      );
    }
      ...
      child: AnimatedBuilder(
        ...
        builder: (BuildContext context, Widget? child) {
          final shouldHideErrorContent =
              widget.inputValidator == null && widget.errorText == null;

          if (shouldHideErrorContent) return onlyFields();

          return AnimatedSize(
            duration: widget.inputAnimationDuration,
            alignment: Alignment.topCenter,
            child: Column(
  ...
  @protected
  bool get hasFocus {
    final isLastPin = selectedIndex == widget.inputSize;
  ...
  Widget _buildError() {
    if (showErrorState) {
      if (widget.errorBuilder != null) {
        return widget.errorBuilder!.call(_errorText, text);
      }
